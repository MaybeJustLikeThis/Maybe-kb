...
| 路径 | 用途 |
|------|------|
| `notes/` | 笔记目录，按知识类型分子目录（`tech-article/`、`document/`、`troubleshooting/`、`design-decision/`、`code-snippet/`） |
| `attachments/` | 附件目录（PDF/图片，Git LFS） |
| `config.toml` | 配置文件，含知识类型定义 |
...
```powershell
# Windows（PowerShell，项目根目录下）
.\scripts\start.ps1                 # 启动前后端，自动检测环境
.\scripts\start.ps1 -OpenBrowser    # 启动后自动打开浏览器
.\scripts\start.ps1 -SkipWatch      # 跳过文件监听，仅启动 API
```

```bash
# macOS / Linux
./scripts/start.sh
KB_SKIP_WATCH=1 ./scripts/start.sh  # 跳过文件监听
```

脚本自动完成：查找 Python → 检查依赖 → 清理端口残留 → 启动前后端 → 健康检查 → 打印地址。
...
```bash
cd web && npm run dev             # Vite dev server on port 3030
```

浏览器打开 `http://127.0.0.1:3030`（一键启动）或 `http://127.0.0.1:8420`（手动启动，生产模式）。

**watch_dir 容错：** 如果配置的 `watch_dir` 目录不存在，`kb serve` 会输出 `[yellow]Warning[/yellow]` 并跳过文件监听，API 服务正常启动。不再因路径问题崩溃。
...
启动后访问 `http://127.0.0.1:8420`，左侧导航包含六个页面：

| 页面 | 路由 | 功能 |
|------|------|------|
| Overview | `/` | 知识库总览：5 统计卡片、类型柱状图、索引健康度、来源项目、内容格式饼图、最近更新 |
| Notes | `/notes` | 笔记列表，支持按分类/标签/知识类型/来源项目过滤和排序 |
| Manage | `/manage` | 深度管理：按类型/分类/标签/来源/索引五个 Tab 切换，快速跳转到对应笔记 |
| Detail | `/note/:fileId` | 查看笔记全文，Markdown 渲染，底部展示语义相关笔记推荐 |
...
首页 Overview 提供知识库全局一览：

- **5 个统计卡片**：Notes / Types / Categories / Tags / Attachments 总数
- **类型分布柱状图**：5 种知识类型（技术文章、踩坑记录、设计决策、代码片段、文档摘要）的数量分布，点击 bar 跳转到对应类型的笔记列表
- **索引健康度面板**：覆盖率百分比 + 文件数 + 向量数
- **来源项目面板**：各项目的笔记数量，支持跳转过滤
- **内容格式饼图**：Markdown / PDF / DOCX 等格式分布（CSS conic-gradient 实现）
- **最近更新**：最新 8 篇笔记，每行显示类型彩色标签

### 5.3 Manage 管理页

`/manage` 页通过 5 个 Tab 提供深度管理视图：

| Tab | 内容 |
|-----|------|
| By Type | 5 种知识类型的明细卡片，含图标、中文标签、关联标签，点击跳转到过滤后的笔记列表 |
| By Category | 分类目录及每类下的笔记数量 |
| By Tag | 标签云，标签越多越大 |
| By Source | 来源项目列表（默认 `kb`，可扩展外部来源如 Hexo、Obsidian） |
| Index | 覆盖率仪表盘 + 文件/向量统计 + 全量重建索引按钮 |
...
kb 使用 Hexo 兼容的 YAML frontmatter + Markdown，新增 `type` 字段用于知识类型分类：
...
`type` 字段对应 `config.toml` 中 `[kb_types.*]` 配置，决定笔记的知识分类：

| type | 标签 | 说明 |
|------|------|------|
| `tech-article` | 技术文章 | 技术分析、教程、原理解析 |
| `troubleshooting` | 踩坑记录 | 问题现象、根因、解决方案 |
| `design-decision` | 设计决策 | 架构选择、取舍理由、上下文 |
| `code-snippet` | 代码片段 | 可复用的代码模式、脚本 |
| `document` | 文档摘要 | PDF、论文、文档的阅读笔记 |

类型在 Dashboard 和 Manage 页面按颜色区分，便于可视化浏览。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/notes` | 列出笔记（支持 `entry_type`、`source_project`、`sort` 参数） |
| POST | `/api/notes` | 创建笔记 |
...
| GET | `/api/tags` | 获取所有标签 |
| GET | `/api/categories` | 获取所有分类（`?with_count=1` 返回含计数） |
| GET | `/api/attachments/stats` | 附件统计 |
| POST | `/api/attachments` | 上传附件 |
| GET | `/api/index` | 索引进度 |
| POST | `/api/index` | 触发索引 |
| GET | `/api/type-distribution` | 知识类型分布（含中文标签） |
| GET | `/api/source-projects` | 来源项目及笔记数量 |
| GET | `/api/content-type-stats` | 内容格式（markdown/pdf/docx）分布 |
| GET | `/api/index-health` | 索引健康度（笔记数/向量数/覆盖率） |
